...
function [A B C D E F G H] = ABCs_arrays(arr1,arr2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%    number of rows. (F)
[rows,cols] = size(arr2);
F = arr2(1:rows/2, 1:cols);

% 6. Transpose "arr2". (G)
G = transpose(arr2);

% 7. Linearize "arr1". "arr1" should go from an MxN array to an (M*N)x1
%    column vector. The new (M*N)x1 vector should be calculated by going
%    down the columns of "arr1". (H)   
H = reshape(arr1, numel(arr1), 1);
